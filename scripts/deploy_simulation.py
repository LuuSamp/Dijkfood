...
Examples:
  ...
  python deploy.py sim --service food_place --action stop
  python deploy.py sim --service courier --action start --desired-count 2
...
SIM_SPECS: Final[dict[str, SimulatorServiceSpec]] = {
    "customer": SimulatorServiceSpec(
        ...
        dockerfile=_PROJECT_ROOT / "services" / "customer_simulation_service" / "Dockerfile",
        cpu="256",
        memory="512",
        desired_count=1,
    ),
    "food_place": SimulatorServiceSpec(
        ...
        docker_context=_PROJECT_ROOT,
        dockerfile=_PROJECT_ROOT / "services" / "food_place_simulation_service" / "Dockerfile",
        cpu="256",
        memory="512",
        desired_count=1,
    ),
    "courier": SimulatorServiceSpec(
        ...
        docker_context=_PROJECT_ROOT,
        dockerfile=_PROJECT_ROOT / "services" / "courier_simulation_service" / "Dockerfile",
        cpu="256",
        memory="512",
        desired_count=1,
    ),
...
def run_simulation(
    *,
    ...
    resolved_base_url = (base_url or base_url_conn or "").strip()
    if not resolved_base_url and action == "deploy":
        raise SystemExit("[deploy_sim] ERROR: BASE_URL is required (arg, env, or connection.env)")
    resolved_tracking = (
        (tracking_base_url or f"{resolved_base_url.rstrip('/')}/tracking").strip()
        if resolved_base_url
        else (tracking_base_url or "").strip()
    )
    ...
    suffix = state.suffix
    service_ids = sorted(SIM_SPECS.keys()) if service == "all" else [service]
    ...
    if action in ("stop", "start"):
        _run_stop_or_start(
            ...
            action=action,
            desired_count_override=desired_count,
        )
        return
    ...
    _run_deploy(
        ...
        tracking_base_url=resolved_tracking,
        exec_arn=exec_arn,
        task_arn=task_arn,
        subnet_ids=subnet_ids,
