...
- Python 3.11+ (3.14 OK for deploy and SageMaker jobs; use 3.11–3.12 for local sklearn inference)
...
Optional ML client (training orchestration, dataset prep, batch predict):
...
python deploy.py --service routing        # rebuild/push one image and roll ECS
```

**`--service`** choices: `ordering` | `tracking` | `routing` | `agent` | `prediction`. Add **`--recreate`** to also reset `desired_count` from deploy specs.
...
```bash
python deploy.py --skip-teardown --with-analytics      # datalake S3, Glue, Streams ingest Lambda, predictions table
python deploy.py --skip-teardown --with-predictions    # analytics + prediction ECS gateway at /prediction/*
```

After deploy, generate traffic, prepare data, train, and wire the endpoint:

```bash
python -m simulator.orchestration.load_test --duration 1800   # needs BASE_URL in connection.env
python -m ml.prepare_datasets
python -m ml.train --deploy-delivery                          # SageMaker jobs; deploys delivery endpoint
python deploy.py --skip-teardown --resume --with-predictions    # roll prediction service with endpoint env
python -m ml.batch_predict                                    # demand + anomaly via Batch Transform
python -m ml.monitor                                          # MAE vs actuals → CloudWatch
python -m ml.teardown_endpoints                               # delete SageMaker endpoints (save budget)
```

Copy `SAGEMAKER_*` lines printed by `ml.train` into `connection.env` if not auto-discovered. Full ML details: [`ml/README.md`](ml/README.md).

The **delivery-time** regressor uses `food_place_id`, `hour`, `weekday`, and **`distance_m`** (routing shortest-path meters). **`POST /place-order`** triggers a prediction after route calculation; results are stored in DynamoDB and served at **`GET /prediction/v1/delivery-time/{order_id}`**.
...
See [`agent/README.md`](agent/README.md) for the HTTP API and tool registry.
...
- **Ordering** (full API): default ALB target (`/health`, `/customers`, …). **`POST /orders`** uses **`order_status_id`** (1–6); **`courier_id`** may be omitted when **`ROUTING_BASE_URL`** is set (nearest idle courier by drive distance using lat/**lon** / initial coordinates). **Workflow routes** (implemented): **`POST /place-order`**, **`POST /assign-courier`**, **`POST /admin/clear-orders-and-logs`**. Placeholder **`/sim/*`** routes return **501**; the load test falls back to **`POST /orders`**, **`PUT /orders/{id}`**, and **`POST /order-logs`**.
- **Tracking** (stub): `http://<alb>/tracking/health`, `/tracking/`.
- **Routing** (OSMnx + NetworkX): `GET /health` (ALB liveness), **`GET /routing/ready`** (503 until the street graph is loaded), **`GET /routing/v1/random-points?n=...`**, **`POST /routing/v1/shortest-path`**, **`POST /routing/v1/nearest-courier`**. Full **`deploy.py`** creates an **S3 bucket** per deployment and sets **`ROUTING_GRAPH_S3_BUCKET`** on the routing task: the first task **downloads OSM** and **uploads a GraphML** snapshot; later starts **load from S3** (much faster). Data © OpenStreetMap contributors ([ODbL](https://www.openstreetmap.org/copyright)).
- **Prediction** (`--with-predictions`): **`GET /prediction/health`**, **`POST /prediction/v1/delivery-time`**, **`GET /prediction/v1/delivery-time/{order_id}`**, **`GET /prediction/v1/demand-forecast`**, **`GET /prediction/v1/anomalies`**, **`GET /prediction/v1/metrics`**.
...
| `ECS_SERVICES_JSON` | JSON array of ECS microservice records (required for teardown; `--resume` can recover from AWS if missing) |
...
| `DYNAMO_ORDER_LOGS_TABLE`, `DYNAMO_COURIER_POSITIONS_TABLE`, `DYNAMO_ROUTES_TABLE` | DynamoDB teardown |
| `DYNAMO_AGENT_SESSIONS_TABLE` | Agent session store (`--with-agent`) |
| `DYNAMO_PREDICTIONS_TABLE` | Stored delivery-time predictions (`--with-analytics`) |
...
|---------|-----------|--------|---------------|
| ordering | default | [`services/ordering/`](services/ordering) (build context: repo root) | Full FastAPI app: CRUD + workflow + DynamoDB |
...
- **Routes** — stores shortest-path results per order (used by ML feature enrichment and workflow).
- **Predictions** — **`orderId`** key; **`predicted_seconds`**, **`source`**, **`distance_m`**, **`features`**, **`updated_at`**.
- **Agent sessions** — conversation history for the Bedrock agent (`--with-agent`).
...
The **ordering** ECS task receives `DYNAMODB_ORDER_LOGS_TABLE`, `DYNAMODB_COURIER_POSITIONS_TABLE`, `DYNAMODB_ROUTES_TABLE`, and `AWS_REGION` / `AWS_DEFAULT_REGION`. With predictions enabled it also gets **`PREDICTION_BASE_URL`** pointing at the same ALB.
...
- [`tools/datalake_infra.py`](tools/datalake_infra.py), [`tools/glue_infra.py`](tools/glue_infra.py), [`tools/analytics_infra.py`](tools/analytics_infra.py) — analytics datalake, Glue catalog, Streams ingest
- [`tools/sagemaker_infra.py`](tools/sagemaker_infra.py) — SageMaker endpoint deploy/teardown helpers
- [`tools/ecs_infra.py`](tools/ecs_infra.py) — ALB, multi-target routing, ECS; teardown does **not** delete IAM roles
- [`ml/`](ml/) — dataset prep, SageMaker training, batch predict, monitoring ([`ml/README.md`](ml/README.md))
- [`services/ordering/`](services/ordering/) — FastAPI ordering service source; OpenAPI UI at `/docs`
- [`simulator/`](simulator/) — simulation service runners + orchestrator; see [`simulator/docs/SERVICES.md`](simulator/docs/SERVICES.md)

## REST API (ordering service)

Each entity has its own router module under [`services/ordering/routers/`](services/ordering/routers/): **POST** (create), **GET** list and **GET** by id (or composite key), **PUT** (replace), **DELETE**.

| Prefix | Module | Storage |
|--------|--------|---------|
| `/customers` | [`customers.py`](services/ordering/routers/customers.py) | RDS (`lat`, `lon` required) |
| `/food-places` | [`food_places.py`](services/ordering/routers/food_places.py) | RDS (enum `kitchen_type_t`) |
| `/couriers` | [`couriers.py`](services/ordering/routers/couriers.py) | RDS (`initial_lat`, `initial_lon` required) |
| `/order-statuses` | [`order_statuses.py`](services/ordering/routers/order_statuses.py) | RDS (read-only list) |
| `/orders` | [`orders.py`](services/ordering/routers/orders.py) | RDS (`order_status_id` FK) |
| `/order-logs` | [`order_logs.py`](services/ordering/routers/order_logs.py) | DynamoDB composite `order_id` + `timestamp` in path; list `?order_id=` |
| `/courier-positions` | [`courier_positions.py`](services/ordering/routers/courier_positions.py) | DynamoDB `courier_id` + `timestamp_ms`; `lat`/`lon` on items |
| `/place-order`, `/assign-courier`, `/admin/clear-orders-and-logs` | [`workflow_orders.py`](services/ordering/routers/workflow_orders.py) | RDS + DynamoDB + routing + optional prediction |
| `/sim` | [`sim_placeholders.py`](services/ordering/routers/sim_placeholders.py) | Placeholder **501** workflow hooks for the simulator |

Shared DB access: [`services/ordering/database.py`](services/ordering/database.py) (Postgres dependency). DynamoDB tables: [`services/ordering/dynamo.py`](services/ordering/dynamo.py).
